...
const Pagination = ({ postsCount, pageNumber, setPageNumber }:any) => {
    ...
    return (
        <div className="mb-12 w-full flex justify-center items-center">
            ...
            <button className={`p-2 px-4 rounded-lg text-white mx-2 ${pageNumber >= totalPages-2 ? "bg-gray-500 hover:cursor-not-allowed hover:bg-gray-600" : "bg-indigo-600 hover:bg-indigo-800 hover:cursor-pointer"}`} onClick={increasePageNumber} disabled={pageNumber===totalPages-2}>Next</button>
        </div>
    );
};
