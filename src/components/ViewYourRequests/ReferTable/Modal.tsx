...
const Modal: React.FC<any> = ({ showModal, handleClose, data, getStatusDisplay }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      ...
      {/* Replicating "REFER" div */}
      <div className="animate-scaleUp2 bg-white rounded-xl shadow-lg w-11/12 md:w-3/4 lg:w-1/2 border-2 border-indigo-700">
        <div className='w-full text-xl font-semibold mb-4 bg-indigo-700 py-2 px-4 text-white'>YOUR REFERRAL REQUESTS</div>
        
        {/* Title and Job Details Section */}
        <h2 className="text-2xl bg-gradient-to-r pl-8 pt-8 from-indigo-700 to-violet-500 inline-block text-transparent bg-clip-text font-bold">DETAILS:</h2>
        ...
        <div className='px-8'>
          ...
          <div className='grid grid-cols-2 gap-4 border-b p-2 border-gray-300'>
        ...
        <div className="flex mt-4">
          <button
            className={`mr-4 my-4 ml-8 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 ${data.referredStatus !== 1 && "bg-gray-700 cursor-not-allowed hover:bg-gray-700"}`}
