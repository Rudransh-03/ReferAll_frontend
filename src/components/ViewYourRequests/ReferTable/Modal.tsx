...
const Modal: React.FC<any> = ({ showModal, handleClose, data, getStatusDisplay }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
      <div className="animate-scaleUp2 bg-white rounded-xl shadow-lg w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-1/3 max-h-[90vh] overflow-y-auto border-2 border-indigo-700">
        <div className='w-full text-xl font-semibold mb-4 bg-indigo-700 py-2 px-4 text-white'>REFER</div>
        
        <h2 className="text-2xl bg-gradient-to-r pl-8 pt-8 from-indigo-700 to-violet-500 inline-block text-transparent bg-clip-text font-bold">
          JOB DETAILS:
        </h2>
        ...
        <div className='px-8'>
          ...
          <p className="text-2xl mb-4 bg-gradient-to-r from-indigo-700 to-violet-500 inline-block text-transparent bg-clip-text font-semibold">
            APPLICANT'S INFO
          </p>
        ...
        <div className="flex mt-4">
          <button
            className={`mr-4 my-4 ml-8 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 ${data.referredStatus !== 1 && "bg-neutral-500	hover:bg-neutral-500 cursor-not-allowed"}`}
