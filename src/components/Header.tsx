  ...
  return (
    <div className="fixed top-0 w-screen bg-indigo-800 text-white p-4 flex px-8 h-24 items-center z-50">
      ...
      <div className="w-1/2 flex justify-around text-xl font-light">
        ...
        <Link className={`hover:cursor-pointer hover:underline underline-offset-4 ${(path === 'my_profile') ? 'underline' : ''}`} to="/my_profile">Profile</Link>
        <Link className={`hover:cursor-pointer hover:underline underline-offset-4 ${(path === 'login') ? 'underline' : ''}`} to="/login">Login</Link>
      </div>
    </div>
  );
};
