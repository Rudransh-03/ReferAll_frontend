...
const ViewJobPosts: React.FC = () => {
  ...
  useEffect(() => {
    const fetchJobPosts = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/referPosts/getAllReferPosts/${userId}`, {
          headers: {
  ...
  return (
    <div className='mt-40'>
      <div className="w-full text-center text-5xl h-16 font-semibold">VIEW <span className="h-16 bg-gradient-to-r from-indigo-700 to-violet-500 inline-block text-transparent bg-clip-text">JOB POSTS</span></div>
